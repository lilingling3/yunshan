...
class XingeApp {
	 ...
	 */
	public static function PushTokenAndroid($accessId, $secretKey, $title, $content, $token,$custom=[])
	{
	 ...
	 * 使用默认设置推送消息给单个ios设备
	 */
	public static function PushTokenIos($accessId, $secretKey, $content, $token, $environment,$custom=[])
	{
		$push = new XingeApp($accessId, $secretKey);
		$mess = new MessageIOS();
	 ...
	 * 使用默认设置推送消息给单个ios版账户
	 */
	public static function PushAccountIos($accessId, $secretKey, $content, $account, $environment,$custom=[])
	{
		$push = new XingeApp($accessId, $secretKey);
		$mess = new MessageIOS();
		$mess->setAlert($content);
	 ...
	 */
	public static function PushAllAndroid($accessId, $secretKey, $title, $content,$custom=[])
	{
		$push = new XingeApp($accessId, $secretKey);
		$mess = new Message();
		$mess->setTitle($title);
