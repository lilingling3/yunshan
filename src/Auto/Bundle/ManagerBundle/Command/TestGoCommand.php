...
class TestGoCommand extends ContainerAwareCommand
{

    public function configure()
    {
        $this
            ->setName('demo:test:wall')
            ->setDescription('test');
    }
    ...
    public function test()
    {
        ...
        if ($member) {
            ...
            foreach ($member as $key => $value) {
                ...
                dump($mobile);
                $name = $this->getContainer()->get('auto_manager.global_helper')->name_protect($name);


                dump($name);
                
                $mobile = $this->getContainer()->get('auto_manager.global_helper')->mobile_protect($mobile);
                dump($mobile);
            }
...
exit;
        ...
        // $car_location_data = $this->getContainer()->get('auto_manager.fee_zu_helper')->findCarLocation('116502100004633');
        // dump($car_location_data);
    }
